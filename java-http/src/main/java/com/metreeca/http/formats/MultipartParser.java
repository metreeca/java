...
package com.metreeca.http.formats;
...
import static com.metreeca.http.Response.PayloadTooLarge;
...
final class MultipartParser {
    ...
    @FunctionalInterface private static interface State {
        ...
        public State next(final Type type) throws FormatException;
    ...
    void parse() throws IOException, FormatException {
    ...
    private State preamble(final Type type) throws FormatException {
    ...
    private State part(final Type type) throws FormatException {
    ...
    private State body(final Type type) throws FormatException {
    ...
    private State header(final State next) throws FormatException {
        try {
    ...
    private State error(final int status, final String message) throws FormatException {
        try {

            throw new FormatException(status, format("%s (%d)", message, body));
    ...
    private Type read() throws IOException, FormatException {
        ...
        for (int c; !(cr == '\r' && lf == '\n') && (c=input.read()) >= 0; cr=lf, lf=c, ++part, ++body, ++last) {
            ...
            if ( part >= partLimit ) {
                error(PayloadTooLarge, format("part size limit <%,d> exceeded", partLimit));
            }

            if ( body >= bodyLimit ) {
                error(PayloadTooLarge, format("body size limit <%,d> exceeded", bodyLimit));
            }
