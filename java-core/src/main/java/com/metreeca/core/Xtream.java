...
public final class Xtream<T> implements Stream<T> {
     ...
     *            value
     *
     * @return an extended stream produced by removing from this extended stream all elements sharing the same
     * {@code key} with previously processed elements
     *
     ...
     * returning a stream of entries mapping each grouping key returned by {@code classifier} to the list of the elements
     ...
     *
     * @return an extended stream containing a single element produced by collecting the elements of this extended stream
     ...
     *
     * @return an extended stream produced by iteratively applying {@code mapper} this this extended stream and replacing
     * it with the elements of the returned streams until {@code steps} cycles are performed; null returned streams are
     ...
     *
     * @param visitor a function mapping elements to streams of elements of the same type
     *
     * @return an extended stream produced by recursively applying {@code visitor} to this extended stream and expanding
     ...
     */
    public Xtream<T> loop(final Function<? super T, ? extends Stream<T>> visitor) { // !!! lazy
        ...
        final Deque<T> pending=stream.distinct().collect(toCollection(ArrayDeque::new));
     ...
     * Recursively expands this extended stream.
     *
     * @param visitor a function mapping elements to streams of map entries whose:
     *
     *                <ul>
     *                  <li>key contains a stream of target elements of the same type as this stream to be recursively
     *                  scanned</li>
     *                  <li>value contains a stream of return elements to be included in the consolidated return
     *                  stream</li>
     *                </ul>
     *
     * @return an extended stream produced by recursively applying {@code visitor} to this extended until no new target
     * elements are generated
     *
     * @throws NullPointerException if {@code visitor} is {@code null}
     */
    public <V> Xtream<V> scan(
            final Function<? super T, Stream<Entry<? extends Stream<T>, ? extends Stream<V>>>> visitor
    ) { // !!! lazy
        ...
        final Collection<T> visited=new HashSet<>();
        final Collection<V> extracted=new HashSet<>();

        final Deque<T> pending=stream.distinct().collect(toCollection(ArrayDeque::new));

        while ( !pending.isEmpty() ) {

            visitor.apply(pending.pop()).forEach(entry -> {
     ...
     *
     * @return an extended stream produced by applying {@code mapper} to this extended stream or an empty extended stream
