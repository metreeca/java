...
final class JSONLDDecoder {
	...
	Stream<Entry<Value, Stream<Statement>>> values(final JsonValue value, final Shape shape) {
		...
		final Set<String> langs=tagged ? langs(shape) : emptySet();
	...
	private Entry<Value, Stream<Statement>> value(final JsonString string, final Shape shape) {
		...
		final IRI type=datatype(shape).orElse(XSD.STRING);

		final Value value=Values.ResourceType.equals(type) ? resource(text)
				: Values.BNodeType.equals(type) ? bnode(text)
				: Values.IRIType.equals(type) ? iri(text)
				: literal(text, type);
	...
	private Entry<Value, Stream<Statement>> resource(final JsonObject object, final Shape shape, final Resource focus) {
		...
		final Map<String, Field> labels=Field.labels(shape, keywords);
		...
		return entry(focus, object.entrySet().stream().flatMap(entry -> {
			...
			if ( label.equals("@type") && value instanceof JsonString ) {
				...
				return Stream.of(Values.statement(focus, RDF.TYPE, iri(((JsonString)value).getString())));
				...
				return values(value, field.shape()).flatMap(pair -> {
					...
					final Statement edge=Values.traverse(field.iri(),

							iri -> Values.statement(focus, iri, target),

							iri -> target instanceof Resource
									? Values.statement((Resource)target, iri, focus)
	...
	private Resource resource(final String id) {
		return id.isEmpty() ? Values.factory().createBNode()
				: id.startsWith("_:") ? Values.factory().createBNode(id.substring(2))
				: Values.factory().createIRI(resolve(id));
	}
