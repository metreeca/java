...
public final class XPath<R> implements Function<Node, R> {
	...
	public static final class Processor {
		 ...
		 * @param xpath the XPath expression to be evaluated against the target node
		 *
		 * @return an optional XPath processor targeting the value generated by evaluating {@code xpath} against the
		 * current target node, if one was available; an empty optional, otherwise
		 *
		 * @throws NullPointerException if {@code xpath} is null
		 */
		public Optional<XPath.Processor> path(final String xpath) {
		 ...
		 * @throws NullPointerException if {@code xpath} is null
		 */
		public Xtream<XPath.Processor> paths(final String xpath) {
		...
		public Xtream<Node> nodes(final String xpath) {
			...
			return Xtream.from(StreamSupport.stream(Spliterators.spliteratorUnknownSize(new Iterator<>() {
