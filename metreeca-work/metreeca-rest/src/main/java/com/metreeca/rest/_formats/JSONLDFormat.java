...
import static com.metreeca.rest._formats.OutputFormat.output;
...
public final class JSONLDFormat extends _Format<Frame> {
     ...
     */
    public static _Either<_MessageException, Query> query(final IRI focus, final Shape shape, final String query) {
    ...
    @Override public <M extends Message<M>> _Either<_MessageException, Frame> decode(final M message) {
        return message

                .header("Content-Type")

                .filter(JSON.MIMEPattern.asPredicate().or(String::isEmpty))
    ...
    /**
     * Configures {@code message} {@code Content-Type} header to {@value JSON#MIME}, unless already defined, and
