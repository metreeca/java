...
package com.metreeca.http.rdf4j.handlers;

import com.metreeca.http.Request;
import com.metreeca.http.Response;
import com.metreeca.http.json.JSONAssert;
...
import static com.metreeca.http.rdf.ModelAssert.assertThat;
import static com.metreeca.http.rdf.Values.statement;
import static com.metreeca.http.rdf4j.services.GraphTest.exec;
import static com.metreeca.http.rdf4j.services.GraphTest.model;
...
final class SPARQLTest {
    ...
    @Test void testPublicAnonymousGETBooleanQuery() {
        exec(model(asList(First, Rest)), () -> _public(endpoint())
                ...
                .map(response -> assertThat(response)
                        .hasStatus(Response.OK)
                        .hasBody(new JSON(), json -> JSONAssert.assertThat(json).satisfies(hasBooleanValue(true)))
                ));
    }
    ...
    @Test void testPrivateAuthenticatedGETTupleQuery() {
        exec(model(asList(First, Rest)), () -> _private(endpoint())
                ...
                .map(response -> assertThat(response)
                        .hasStatus(Response.OK)
                        .hasBody(new JSON(), json -> JSONAssert.assertThat(json).satisfies(hasBindings(RDF.FIRST,
                                RDF.REST)))
                ));
    }
    ...
    @Test void testPrivateAuthenticatedGETGraphQuery() {
        exec(model(asList(First, Rest)), () -> _private(endpoint())
                ...
                .map(response -> assertThat(response)
                        .hasStatus(Response.OK)
                        .hasBody(new com.metreeca.http.rdf.formats.RDF(), rdf -> assertThat(rdf)
