...
public final class ShiftEvaluator extends Shift.Probe<Stream<Frame>> {
    ...
    @Override public Stream<Frame> probe(final Shift.Step step) {
        return frame.frames(step.iri());
    }

    @Override public Stream<Frame> probe(final Shift.Seq seq) {

        Function<Stream<Frame>, Stream<Frame>> pipe=identity();

        for (final Shift.Path path : seq.paths()) {
    ...
    @Override public Stream<Frame> probe(final Shift.Alt alt) {
