...
final class JSONLDParser {
	...
	private Shape filter(final JsonValue value, final Shape shape,
		...
		return Optional
				...
				.ofNullable(shape.map(new Shape.Probe<Shape>() {
					...
					@Override public Shape probe(final Field field) {
						return path.isEmpty() || !field.iri().equals(path.get(0)) ? null : Optional
								...
								.map(s -> Field.field(field.label(), field.iri(), s))
								.orElse(null);
					}
				...
				.orElseGet(() -> {
					...
					if ( !path.isEmpty() ) {
						throw new NoSuchElementException(format("unknown path step %s", Values.format(path.get(0))));
					}
	...
	private Order criterion(final String criterion) {
		return criterion.startsWith("+") ? Order.increasing(path(criterion.substring(1), shape))
				: criterion.startsWith("-") ? Order.decreasing(path(criterion.substring(1), shape))
				: Order.increasing(path(criterion, shape));
	}
