...
final class JSONLDTest {
    ...
    @Nested final class Decoder {
        ...
        private Request request(final String json) {
            return shape(new Request().base(base)
    ...
    @Nested final class Encoder {
        ...
        private Response response(final Response response) {
            ...
            return shape(response.status(OK), And.and(
        ...
        @Test void testLocalizePayload() {
            exec(() -> new Request()
                    ...
                    .base(base)
                    .header("Accept-Language", "en").reply().map(response -> {

                        final IRI item=iri(response.item());

                        return shape(response.status(OK), Field.field(direct, Localized.localized()))
        ...
        @Test void testReportInvalidPayload() {
            exec(() -> assertThatExceptionOfType(RuntimeException.class).isThrownBy(() ->

                    request().reply()
                            .map(response -> response(response).body(new JSONLD(), frame(iri(response.item()))))
                            .map(task(response -> response.output().accept(new ByteArrayOutputStream())
                            ))));
